...
export default function landingScreen({ navigation }) {
  ...
  function savePicker(value) {
    try {
      ...
      console.warn("in savePicker - error when saving: ", err);
    }
  }
  ...
  function savePassword_Username() {
    try {
      ...
      console.warn("in savePass... - error when saving: ", err);
    }
  }
  ...
  function navigate() {
    if (isConnected) {
      ...
      fetch(`${url}/timetables?username=${username}&password=${password}`)
        .then((data) =>
          data.json().then((json) => {
            // .then( () => {} )
      ...
      Alert.alert(
        ...
        "Bitte überprüfe deine Internetverbindnug."
      );
  }
  ...
  return (
    <View style={stylesLanding.container}>
      ...
      <View style={stylesLanding.headerWrapperWrapper}>
        ...
        <View>
          <Text style={stylesLanding.header2}>
            Wähle deine Klasse (aktuell: {classes})
          </Text>
        </View>
      </View>
